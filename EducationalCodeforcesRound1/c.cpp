...
typedef struct _vector {
	int id;
	complex<long double> pt;
	long double ang;

	const bool operator<(const struct _vector &op) const {
		return ang < op.ang;
	}
} Vector;
...
int main(int argc, char *argv[])
{
	...
	fori (i, 1, n) {
		...
		pts[i].id = i;
		pts[i].pt = complex<long double>(x, y);
		pts[i].ang = arg(pts[i].pt);
	}
	...
	fori (i, 1, n) {
		u = i;
		v = i+1;
		nonoriang = (pts[v].ang - pts[u].ang);
		if (nonoriang < 0) {
			nonoriang = 2*PI + nonoriang;
		}

		if (chkmin(mintheta, nonoriang)) {
			mid[0] = pts[u].id;
			mid[1] = pts[v].id;
		}
	}
