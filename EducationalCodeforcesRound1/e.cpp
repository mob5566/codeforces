...
int minbreak(int a, int b, int c)
{
	if (-1 != dp[a][b][c]) {
		goto END;
	}
	...
	fori (sg, 1, b/2) {
		fori (rg, 0, min(a * sg, c)) {
			chkmin(dp[a][b][c],
				(minbreak(a, sg, rg) + minbreak(a, b-sg, c-rg) + a*a));
		}
	}

	fori (sg, 1, a/2) {
		fori (rg, 0, min(b * sg, c)) {
			chkmin(dp[a][b][c],
				(minbreak(sg, b, rg) + minbreak(a-sg, b, c-rg) + b*b));
		}
	}
END:
	return dp[a][b][c];
