...
int walkmaze(int u, int v, int ru, int rv)
{
	if (vis[u][v]) {
		return 0;
	}
	...
	fat[mkp(u, v)] = mkp(ru, rv);
	int ret = 0;

	fori (d, 0, 3) {
		if (maze[u+dir[d][0]][v+dir[d][1]] == '*') {
			ret += 1;
		} else {
			ret += walkmaze(u + dir[d][0], v + dir[d][1], ru, rv);
		}
	}
