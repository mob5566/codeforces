...
void dfs(int u, int v, int uc, int vc)
{
	int w;
...
int main(int argc, char *argv[])
{
	...
	md = 0;
	fori (i, 1, a) {
		chkmax(md, (int)ae[i].size());
	}
	fori (i, 1, b) {
		chkmax(md, (int)be[i].size());
	}

	fori (u, 1, a) {
		for (auto v : ae[u]) {
			if (!color[idx[mkp(u, v)]]) {
				fac = firstValid(ac[u]);

				if (!bc[v][fac]) {
					ac[u][fac] = v;
					bc[v][fac] = u;
					color[idx[mkp(u, v)]] = fac;
				} else {
					memset(vis, 0, sizeof(vis));
					vis[u] = true;
					dfs(u, v, fac, firstValid(bc[v]));
				}
			}
		}
	}
